...
```ts
  ...
  5  |     const placeHolder = await page.locator("//input[@id='user-message']").innerText();
  6  |     console.log(placeHolder);
  7  |     await page.fill("//input[@id='user-message']","Jerish");
  8  |     await page.click("//button[@id='showInput']");
  9  |     await page.waitForTimeout(2000);
> 10 |     await expect(page.locator("#message")).toContainText("Jerish")
     |                                            ^ Error: expect(locator).toContainText(expected) failed
  11 | });
```