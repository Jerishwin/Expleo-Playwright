...
- Name: skipDemo.test.ts >> Login Test 1
- Location: tests\skipDemo.test.ts:3:1

# Error details

```
Error: page.click: Unexpected token "#" while parsing css selector "#//button[text()='Log in']". Did you mean to CSS.escape it?
Call log:
  - waiting for #//button[text()='Log in']
...
> 9  |     await page.click("#//button[text()='Log in']")
     |                ^ Error: page.click: Unexpected token "#" while parsing css selector "#//button[text()='Log in']". Did you mean to CSS.escape it?
  10 | 
  11 |     await expect.soft(page.getByRole("link",{name:"Log out"})).toBeVisible()
  12 |     await expect.soft(page.getByRole("link",{name:"Log out"}).textContent(),"Log out")
  13 | 
  14 | 
  15 | })
  16 | 
  ...
  21 |     await page.fill("#loginusername","Banton24")
  22 |     await page.fill("#loginpassword","JeriJose.1")
  23 |     await page.click("#//button[text()='Log in']")
  24 | 
  25 |     await expect.soft(page.getByRole("link",{name:"Log out"})).toBeVisible()
  26 |     await expect.soft(page.getByRole("link",{name:"Log out"}).textContent(),"Log out")
  27 | 
  28 |     
  29 | })
  30 | 
  31 | test("Login Test 3",async({page})=>{
  32 |     await page.goto("https://demoblaze.com/")
  33 | 
  34 |     await page.click("#login2")
  35 |     await page.fill("#loginusername","Banton24")
  36 |     await page.fill("#loginpassword","JeriJose.1")
  37 |     await page.click("#//button[text()='Log in']")
  38 | 
  39 |     await expect.soft(page.getByRole("link",{name:"Log out"})).toBeVisible()
  40 |     await expect.soft(page.getByRole("link",{name:"Log out"}).textContent(),"Log out")
  41 | 
  42 |     
  43 | })
```